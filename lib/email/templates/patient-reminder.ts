...
const STRINGS = {
  fr: {
    subject: (serviceName: string, clinicName: string) =>
      `Rappel — Votre rendez-vous demain : ${serviceName} chez ${clinicName}`,
  ...
  en: {
    subject: (serviceName: string, clinicName: string) =>
      `Reminder — Your appointment tomorrow: ${serviceName} at ${clinicName}`,
    heading: "Appointment Reminder ⏰",
    greeting: (name: string) => `Hi <strong>${name}</strong>,`,
    intro: "This is a reminder for your appointment scheduled tomorrow:",
    labelService: "Service",
...
export function buildPatientReminderSubject(
  serviceName: string,
  clinicName: string,
  locale: EmailLocale
): string {
...
export function buildPatientReminderHtml(data: PatientReminderData): string {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;padding:60px 20px;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:24px;overflow:hidden;box-shadow:0 10px 40px -10px rgba(0,0,0,0.08);border:1px solid #f1f5f9;">
        ...
        <tr>
          <td style="background:#1e3a5f;padding:48px 40px;text-align:center;">
                ...
                <td style="padding:24px 32px;">
                  <table width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      ...
                      <td style="padding:12px 0;border-bottom:1px solid #f8fafc;text-align:right;">
                        <strong style="color:#1e3a5f;font-size:14px;font-weight:600;">${formattedTime}</strong>
                      </td>
                    </tr>
                    <tr>
